...
Since the two variables that I am using are all treated as continuous
variable, I will not be convert them to factor variable. Just in case, I
drop all the missing value among them.
...
smoked per day during pregnancy as my predictor to build my model. Also,
I will use `wtgain` mother’s weight gain during pregnancy (pounds). This
variable might suggest how well the mother’s diets are during pregnancy.
...
the all of the interactions terms. It has much lower `rmse` value. My
model is the worst, it has the highest `rmse`.
